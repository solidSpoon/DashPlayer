...
> 点击<control>下载</control>后，应用会自动探测官方地址与国内备用镜像（hf-mirror.com）的可达性，并从可达的地址断点续传下载。两个地址都不可达时会提示检查网络，此时可在模型区域展开手动下载指引：指引中同时列出官方与镜像地址，用浏览器下载模型文件后保存到指定目录，再回到这里点击"下载"，应用会继续断点续传或直接安装。
>
> {style="note"}
