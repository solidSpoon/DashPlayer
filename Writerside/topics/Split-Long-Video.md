...
在侧边栏的 <control>切分长视频</control> 页面，您可以对长视频切割，生成多个短视频，放在原视频的同名文件夹下。

## 如何使用

先点击 <control>选择文件</control> 选中要切分的视频，然后输入 `时间戳 标题` 格式的文本，一般在视频的评论区或者简介中会有这样的格式。
...
您输入后可在右边的预览区域看到识别结果，点击 `开始切分` 按钮即可生成短视频。
...
复制这段文本到切分长视频页面的文本输入框中，整理后的时间戳如下：
