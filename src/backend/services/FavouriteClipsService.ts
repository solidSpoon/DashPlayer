...
@injectable()
export class FavouriteClipsServiceImpl implements FavouriteClipsService {
    ...
    private mapToClipKey(srtKey: string, indexInSrt: number): string {
        ...
        const { contentSrt } = buildClipContext(srt.sentences, indexInSrt);
        return hash(contentSrt);
    }
