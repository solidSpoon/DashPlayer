...
import { LOCAL_SUBTITLE_STRATEGY_VERSION } from '@/backend/infrastructure/translate/subtitleBatchPrompt';

/**
 * 字幕翻译缓存的持久化模式。
 *
 * 除 'tencent' 外统一用 '#' 分段：`openai#模型#模式#风格签名`、
 * `local#策略版本#模型#模式#风格签名` 与 `local-mt#模型#模式#风格签名`。
 * 不用 '_' 分段是因为模型 ID 自带下划线（如 qwen3-0.6b-q4_k_m-v1），
 * 混在一起无法辨认分段边界。
 */
 ...
 * 避免同一配置在多处重复拼装后产生不一致。
 *
 * 本地引擎额外编入策略版本：提示词/解码策略升级会改变同一源文的期望
 * 译文，版本随 subtitleBatchPrompt 的策略进位，进位后旧策略的缓存自然
 * 不再命中（旧分区行成为死数据，量小可接受）。
 *
...
export const buildSubtitleStorageMode = (
    ...
    signature: string
): SubtitleTranslationStorageMode => engine === 'local'
    ? `local#v${LOCAL_SUBTITLE_STRATEGY_VERSION}#${modelId}#${mode}#${signature}`
    : `${engine}#${modelId}#${mode}#${signature}`;
