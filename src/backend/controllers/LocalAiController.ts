...
export class LocalAiController implements Controller {
    ...
    /** 注册状态、下载、取消、删除和生成检查路由。 */
    public registerRoutes(): void {
        registerRoute('local-ai/status', () => this.localAi.getStatus());
        registerRoute('local-ai/download', () => this.localAi.download());
        registerRoute('local-ai/cancel-download', () => this.localAi.cancelDownload());
        registerRoute('local-ai/delete', () => this.localAi.deleteModel());
        registerRoute('local-ai/check', async () => {
            const startedAt = Date.now();
            const result = await this.localAi.generate('Translate "Good morning" into Simplified Chinese. Return JSON with a translation field.', {
