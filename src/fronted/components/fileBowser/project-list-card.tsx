        ...
        >
            <div className={cn('relative w-full rounded-lg overflow-hidden border-none')}>
                {url ? <img
                    ...
                    className={cn("w-full object-cover", hover && 'filter brightness-75')}
                ...
                /> : <div
                    ...
                    <div className={cn('absolute bottom-2 right-2 text-white bg-black bg-opacity-80 rounded-md p-1 py-0.5 text-xs flex')}>
                {proj.project_type === WatchProjectType.FILE ? TimeUtil.secondToTimeStrCompact(video?.duration) : <><ListVideo className={'w-4 h-4 mr-1'}/>{`${projDetail?.videos?.length} videos`}</>}
                </div>
                <Progress
