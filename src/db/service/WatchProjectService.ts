...
const findSubtitle = (videoName: string, subtitleFilesList: string[]): string | undefined => {
    const subtitleFile = subtitleFilesList.find((subtitleFileName) => subtitleFileName.startsWith(videoName));
    if (subtitleFile) {
        return subtitleFile;
    }
    const videoNameWithoutExt = videoName.substring(0, videoName.lastIndexOf('.'));
    return subtitleFilesList.find((subtitleFileName) => subtitleFileName.startsWith(videoNameWithoutExt));
};
...
const selectFiles = async (): Promise<WatchProjectVO | undefined> => {
    ...
    const videoFilesList = files.filePaths.filter((fileName) => isVideo(fileName));
    if (videoFilesList.length === 0) {
        return undefined;
    }
    const subtitleFilesList = files.filePaths.filter((fileName) => isSubtitle(fileName));
    const videoFile = videoFilesList[0];
    console.log('selectFiles', subtitleFilesList, videoFile);
    const res: WatchProjectVO = {
        project_name: path.basename(videoFile).substring(0, videoFile.lastIndexOf('.')),
    ...
    res.videos.push({
        ...
        subtitle_path: subtitleFile ? subtitleFile : undefined,
        current_time: 0,
        duration: 0
    });
...
const selectDirectory = async (): Promise<WatchProjectVO | undefined> => {
    ...
    let res: WatchProjectVO | undefined = undefined;
    ...
    const subtitleFilesList = filesList.filter((fileName) => isSubtitle(fileName));
    ...
    videoFilesList.forEach((fileName) => {
        ...
        res?.videos.push({
            ...
            subtitle_path: subtitleFile ? path.join(filePath, subtitleFile) : undefined,
            current_time: 0,
            duration: 0
        });
    });
...
export default class WatchProjectService {
    ...
    public static selectFiles = async (isDirectory: boolean): Promise<WatchProjectVO | undefined> => {
        const select = isDirectory ? await selectDirectory() : await selectFiles();
        if (!select) {
            return undefined;
        }
    ...
    public static async listRecent(): Promise<WatchProjectVO[]> {
        ...
        const watchProjects = await knexDb<WatchProject>(WATCH_PROJECT_TABLE_NAME)
            .select('*')
            .orderBy('updated_at', 'desc')
            .limit(50);
        const watchProjectIds = watchProjects.map((wp) => wp.id);
        const watchProjectVideos = await knexDb<WatchProjectVideo>(WATCH_PROJECT_VIDEO_TABLE_NAME)
            .select('*')
    ...
    private static async tryCreate(select: WatchProjectVO): Promise<WatchProjectVO> {
        const watchProject = await knexDb<WatchProject>(WATCH_PROJECT_TABLE_NAME)
            .select('*')
        ...
        const insertWatchProject = await knexDb<WatchProject>(WATCH_PROJECT_TABLE_NAME)
            .insert({
